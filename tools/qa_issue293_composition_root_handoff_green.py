...
GUI = Path("gui.py")
COMPOSITION = Path("gui_modules/editors/hole_editor_composition.py")

composition = r'''"""Composition root for the unified hole editor.
...
import_anchor = "from gui_modules.editors.hole_editor_render import HoleEditorCanvasRenderer as _HoleEditorCanvasRenderer\n"
composition_import = (
    "from gui_modules.editors.hole_editor_composition import "
    "HoleEditorCompositionDependencies as _HoleEditorCompositionDependencies, "
    "open_unified_hole_editor as _open_unified_hole_editor_impl\n"
)
if composition_import not in gui:
    if gui.count(import_anchor) != 1:
        raise SystemExit(f"composition import anchor mismatch: {gui.count(import_anchor)}")
    gui = gui.replace(import_anchor, composition_import + import_anchor, 1)

start = gui.index("    def _open_unified_hole_editor(\n")
end = gui.index("    def open_hole_editor(self, key):\n", start)
replacement = '''    def _open_unified_hole_editor(self, *args, **kwargs):
...
'''
gui = gui[:start] + replacement + gui[end:]
