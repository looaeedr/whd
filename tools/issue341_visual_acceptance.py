...
LEFT_WIDTH_FLOORS = {"small": 338, "medium": 430, "large": 480}
CHECKS = (
    "Hierarchy",
    "Spacing",
    "Clipping",
...
def probe_geometry_reachability(root, designer, scale):
    ...
    left_width = int(designer.left_scroll_canvas.winfo_width())
    renderer_width = int(designer.renderer.canvas.get_tk_widget().winfo_width())
    width_floor_ok = left_width >= LEFT_WIDTH_FLOORS[scale]
    renderer_floor_ok = renderer_width >= 760
    failures = [row for row in records if not row["pass"]]
    if not width_floor_ok:
        failures.append({
            "name": "left_workspace_width",
            "actual": left_width,
            "required": LEFT_WIDTH_FLOORS[scale],
        })
    if not renderer_floor_ok:
        failures.append({
            "name": "renderer_viewport_width",
            "actual": renderer_width,
            "required": 760,
        })
    return {
        "scale": scale,
        "root_rect": viewport,
        "left_workspace_width": left_width,
        "left_workspace_width_floor": LEFT_WIDTH_FLOORS[scale],
        "left_workspace_width_ok": width_floor_ok,
        "renderer_viewport_width": renderer_width,
        "renderer_viewport_width_floor": 760,
        "renderer_viewport_width_ok": renderer_floor_ok,
        "records": records,
        "pass": not failures,
        "failures": failures,
    }
...
def probe_effective_styles(root, designer, scale):
    ...
    mpl_text = [
        {
            ...
            "resolved_font_path": str(
                font_manager.findfont(artist.get_fontproperties(), fallback_to_default=True)
            ),
        }
    ...
    cjk_font_ok = any(
        any(name in row["resolved_font_path"].replace("\\", "/").lower()
            for name in ("noto", "jhenghei", "pingfang"))
        for row in mpl_text
        if row["text"].strip()
    )
    mpl_row = {
        ...
        "pass": bool(mpl_text and text_ok and line_ok and cjk_font_ok),
    }
