...
def test_issue348_joint_world_geometry_has_one_canonical_owner():
    import phase6_manufacturing_geometry as owner
    import fold_designer_bridge as bridge

    name = "_phase6_build_joint_world_geometry"
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.Import):
            reverse += [a.name for a in node.names if a.name == "fold_designer_bridge"]
