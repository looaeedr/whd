...
```text
python tools/execution_claim_guard.py --claim <shared-claim-json> --issue <N> --worker <identity> --branch <branch> --action <branch-create|write|commit|qa-dispatch|workflow-dispatch|pr-write>
```
...
- [ ] `EXECUTION_CLAIM_PREWRITE_HARD_GATE`：atomic claim 取得後仍須在 `branch-create` / repo write / `qa-dispatch` 前執行 `tools/execution_claim_guard.py`；missing/non-owner/mismatch/malformed/stale/ambiguous 一律 fail closed。
