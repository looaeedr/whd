...
- [ ] `X_SECOND_MAIN_INDEPENDENT_CHAIN_CONTRACT`：每個 Master 固定 `FROZEN_X_BASE_SHA`；active chain 不追 current X；A/B/C/D 兄弟鏈隔離；child parent = previous accepted HEAD；整鏈 Task Acceptance 完成後才進 Integration Acceptance；`READY_FOR_X_INTEGRATION` 仍受 `DO_NOT_MERGE_X` / explicit authorization gate 約束。
