---
whd_doc_role: HISTORICAL
whd_contract: handoff-provenance
whd_canonical: null
whd_schema: WHD_DOC_META_V1
---
<!-- WHD_DOC_ROLE role=MIRROR contract=handoff-entry POINTER_ONLY -->
# 00 — Handoff Compatibility Entry

> **[MIRROR / POINTER_ONLY]** 此路徑只保留舊入口相容性，不複製 current authority。

Current entry / authority：
- `../AGENTS.md` — Agent 啟動與 Preflight process。
- `../個人AI檔案庫/第二層_專案與SOP/09_WHD_Canonical_Authority_Map.md` — CURRENT / REFERENCE / MIRROR / HISTORICAL ownership map。
- `../個人AI檔案庫/第二層_專案與SOP/04_WHD鈑金展開幾何引擎規範.md` — 現行 ae_engine manufacturing / Certified Registry contract。

`handoff/01_ARCHITECTURE.md`、`handoff/05_NEXT_STEPS.md` 等舊 handoff 檔保留為 historical snapshot；不得再由本 mirror 把它們升格為 current architecture / roadmap。
