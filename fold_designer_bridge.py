...
def _phase6_build_visual_controls(self, parent):
    self.visual_controls = original.ttk.Frame(parent, padding=5)
...
def _phase6_build_output_controls(self, parent=None):
    """Compact top-row Output controls; existing state/callback owners are unchanged."""
    host = parent or self.top_command_row
    self.output_controls_frame = original.ttk.Frame(host, padding=(4, 0))
    self.output_controls_frame.pack(
        side=original.tk.LEFT, fill=original.tk.X, expand=True, padx=(4, 0)
    )
    ...
    for index, (key, label) in enumerate(labels):
        ...
        check.pack(side=original.tk.LEFT, padx=(0, 6))
...
def _phase6_build_persistent_top_area(self):
    ...
    # UI v3: File and existing Output controls share one compact top command row.
    ...
    # Display/global controls stay with the drawing workspace on the right.
    ...
    self.right_global_host.pack(fill=original.tk.X, pady=(2, 0))
